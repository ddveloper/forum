...
'''
    Get the header from the request, raise an AuthError if no header is present
    Attempt to split bearer and the token, raise an AuthError if the header is malformed
...
'''
    ...
    @INPUTS
        toekn: a json web token (string)

    toekn should be an Auth0 token with key id (kid)
